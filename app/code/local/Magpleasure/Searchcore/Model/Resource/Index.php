...
class Magpleasure_Searchcore_Model_Resource_Index extends Mage_Core_Model_Mysql4_Abstract
{
     ...
     * @return $this
     */
     public function reindexItem($object, $config, $isFullIndex = false)
     {
        if(!$object instanceof Mage_Core_Model_Abstract){
            // Mage::log('Magpleasure_Searchcore_Model_Resource_Index::reindexItem::object', null, 'debug_ipune.log', true);
            // Mage::log(@get_class($object)?: '', null, 'debug_ipune.log', true);
            // Mage::log(json_encode($object, JSON_PRETTY_PRINT), null, 'debug_ipune.log', true);
            return $this;
        }
        if(!$config instanceof Magpleasure_Searchcore_Model_Type_Config){
            // Mage::log('Magpleasure_Searchcore_Model_Resource_Index::reindexItem::config', null, 'debug_ipune.log', true);
            // Mage::log(@get_class($config)?: '', null, 'debug_ipune.log', true);
            // Mage::log(json_encode($config, JSON_PRETTY_PRINT), null, 'debug_ipune.log', true);
            return $this;
        }
    ...
    public function deleteIndex($object, $config)
    {
        if(!$object instanceof Mage_Core_Model_Abstract){
            return $this;
        }
