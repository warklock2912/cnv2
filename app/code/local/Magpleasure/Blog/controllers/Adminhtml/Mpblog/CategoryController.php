    ...
    public function saveAction()
    {
        ...
        if (isset($_FILES['images']['name']) && $_FILES['images']['name'] != '') {
            try {
                ...
                $new_image_name = Mage::helper('mpblog/data_categories')->renameImage($image_name);
            ...
            $requestPost['images'] = $urlpath . DS . $new_image_name;
        } else {
            if (isset($requestPost['images']['delete']) && $requestPost['images']['delete'] == 1) {
                $requestPost['images'] = '';
                //$category->setData
            } else {
                unset($requestPost['images']);
            }
        }

        if (isset($_FILES['image_for_app']['name']) && $_FILES['image_for_app']['name'] != '') {
            try {
                //rename image in case image name has space
                $image_name = $_FILES['image_for_app']['name'];
                $new_image_name = Mage::helper('mpblog/data_categories')->renameImage($image_name);

                $uploader = new Varien_File_Uploader('image_for_app');
                $uploader->setAllowedExtensions(array('jpg', 'jpeg', 'gif', 'png'));
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(false);
            ...
            //var_dump($requestPost); die;
            $urlpath = DS . 'mpblog_category' . DS . 'images';
            $requestPost['image_for_app'] = $urlpath . DS . $new_image_name;
        } else {
            if (isset($requestPost['image_for_app']['delete']) && $requestPost['image_for_app']['delete'] == 1) {
                $requestPost['image_for_app'] = '';
                //$category->setData
            } else {
                unset($requestPost['image_for_app']);
            }
        }
