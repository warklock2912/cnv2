...
class Magpleasure_Blog_Block_Adminhtml_Category_Edit_Tab_General extends Magpleasure_Blog_Block_Adminhtml_Filterable_Widget_Form implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    /**
     * Helper
    ...
    protected function _prepareForm()
    {
        ...
        $categoryType = Magpleasure_Blog_Model_CategoryForApp::getOptionArray();
        $fieldset->addField('category_for_app', 'select',
            array(
                'label' => Mage::helper('mpblog')->__('Category For App'),
                'name' => 'category_for_app',
                'values' => $categoryType,
            ));
        $fieldset->addField('image_for_app', 'image',
            array(
                'label' => Mage::helper('mpblog')->__('Image For App'),
                'required' => false,
                'name' => 'image_for_app',
                'style' => 'display: inline; width: 200px;clear:both;',
                'class' => 'input-file',
        ...
        $fieldset->addField('url_for_app', 'text', array(
            'label' => $this->_helper()->__('URL For App'),
            'name' => 'url_for_app',
        ));
        ...
        if (isset($data['image_for_app']) && $data['image_for_app'] != '') {
            $data['image_for_app'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'mpblog_category/images/' . $data['image_for_app'];
        }
