...
class Tigren_CardId_Adminhtml_CustomerController extends Mage_Adminhtml_CustomerController{
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            $customerAccountInfo = $data['account'];
            if (isset($customerAccountInfo['card_id'])) {
                $customersCardInfoCollections = Mage::getModel('activity/activity')->getCollection()
                    ->addFieldToFilter('customer_id',$customer->getId());
                if (count($customersCardInfoCollections)){
                    foreach ($customersCardInfoCollections as $customersCardInfo){
                        $cardModelId = $customersCardInfo['id'];
                        $cardModel = Mage::getModel('activity/activity')->load($cardModelId);
                        $cardModel->setData('card_id', $customerAccountInfo['card_id']);
                        $cardModel->save();
                    }
                } else {
                    $newCardModel = Mage::getModel('activity/activity');
                    $data = array(
                        "customer_id" => $customer->getId(),
                        "card_id" => $customerAccountInfo['card_id']
                    );
                    $newCardModel->setData($data);
                    $newCardModel->save();
                }
            }
