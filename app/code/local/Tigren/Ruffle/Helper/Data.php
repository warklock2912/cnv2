...
class Tigren_Ruffle_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function generateRuffleNumber($ruffle)
    {
        ...
        do {
            $number = $_prefix . '-' . $ruffle->getId() . '-' . substr(time(), 2). str_pad(mt_rand(000, 999),3,0,STR_PAD_LEFT);
    ...
    public function countUsedQuotaByProductId($productId,$options, $ruffleId)
    {
        $joiners = Mage::getModel('ruffle/joiner')->getCollection();

        $joiners->addFieldToFilter('ruffle_id', $ruffleId)
            ->addFieldToFilter('is_winner', 1)
            ->addFieldToFilter('product_id',$productId)
            ->addFieldToFilter('product_options', $options);
        $usedQuota=count($joiners->getData());
        return $usedQuota;
    }

    public function getUsedQuotaCollectionAll($productId, $ruffleId)
    {
        $usedQuotaCollection = Mage::getModel('ruffle/joiner')->getCollection();
        $usedQuotaCollection->addFieldToFilter('is_winner', 1)
            ->addFieldToFilter('ruffle_id', $ruffleId)
    ...
    public function checkWinnerRuffle($product, $customer_id)
    {
        ...
        $cacheId = 'CACHE_CHECK_WINNER_RUFFLE';
        $cacheTag = 'block_html';
        $dataWinner = [];

        //check if our example_id cache contains any data - load() method will return false if cache is empty
        if (($dataWinner = Mage::app()->getCache()->load($cacheId))) {
            //if cache was found then unserialize it and assign to our variable
            $dataWinner = unserialize($dataWinner);
        } else {
            //if not then normally assign data to the variable
            $resource = Mage::getSingleton('core/resource');
            $read = $resource->getConnection('core_read');
            $query = 'SELECT ruffle_id, customer_id, product_id , product_options  FROM ruffle_joiner WHERE is_winner = 1;';
            $results = $read->fetchAll($query);
            $dataWinner = [];
            foreach($results as $r){
              $dataWinner[$r['customer_id'].'|'.$r['product_id']] = $r;
            }
            //then serialize and save it
            Mage::app()->getCache()->save(serialize($dataWinner), $cacheId, [$cacheTag], 60*60*1);
        }

        $check = @$dataWinner[$customer_id.'|'.$product->getId()] ?: [];
        if(empty($check)){
          return null;
        }else{
          return $check ;
...
//    public function getStatusDocInvoice()
//    {
//        return Mage::getStoreConfig('ruffle/email_settings/enable_doc_invoice_no',Mage::app()->getStore());
//    }
