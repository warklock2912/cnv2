...
class Magebuzz_Bannerads_Adminhtml_Bannerads_ImagesController extends Mage_Adminhtml_Controller_action {
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (isset($_FILES['banner_image']['name']) && $_FILES['banner_image']['name'] != '') {
                ...
                $path = Mage::getBaseDir('media') . DS . "banners" . DS . "images" . DS;
                if (!file_exists($path)) {
            ...
            if ($model->getStartTime()) {
                $isoDate = Mage::helper('bannerads')->getIsoDate($model->getStartTime());
                $stTime = strtotime($isoDate);
                $startTime = date('Y-m-d H:i:s', Mage::getSingleton('core/date')->gmtTimestamp($stTime));
                $model->setStartTime($startTime);
            }
            if ($model->getEndTime()) {
                $isoDate = Mage::helper('bannerads')->getIsoDate($model->getEndTime());
                $stTime = strtotime($isoDate);
                $endTime = date('Y-m-d H:i:s', Mage::getSingleton('core/date')->gmtTimestamp($stTime));
            ...
            try {
                ...
                } elseif (!empty($data['from_block_id'])) {
                    $blockIds = array($data['from_block_id']);
                    Mage::getModel('bannerads/bannerblock')->saveBlock($blockIds, $bannerId);
                }
                ...
                if ($this->getRequest()->getParam('back')) {
                    if (!empty($data['from_block_id'])) {
                        $this->_redirect('*/bannerads/edit', array('id' => $this->getRequest()->getParam('from_block_id')));
                        return;
                    }
