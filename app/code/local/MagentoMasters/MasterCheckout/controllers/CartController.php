 ...
 */
class MagentoMasters_MasterCheckout_CartController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function testAction(){
        $product = Mage::getModel('catalog/product')->load(58230);
        ...
        foreach ($categories as $key => $value) {
            ...
            if($category->getData('counting_down_category')){
            	if($category->getData('counting_downs')){
            		$now = date("Y-m-d H:i:s", Mage::getModel('core/date')->timestamp(time()));
            		$counting_downs = date("Y-m-d H:i:s", strtotime($category->getData('counting_downs')));
	            	if($now > $counting_downs){
	            		echo "OK";
	            	} else {
	            		echo "Please be patient, the product will be available soon.";
	            	}
            	}
            }
        }
