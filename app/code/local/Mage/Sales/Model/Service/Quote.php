...
class Mage_Sales_Model_Service_Quote
{
    ...
    public function submitOrder()
    {
        ...
        $order = null;
        $ppNewFlow = false;
        if($payment->getMethod() == Mage_Paypal_Model_Config::METHOD_WPP_EXPRESS){
        ...
        if($ppNewFlow){
            $order->getPayment()->addData($this->_convertor->paymentToOrderPayment($quote->getPayment())->getData());
        }else {
        ...
        if(!$ppNewFlow || false) {
            ...
            foreach ($quote->getAllItems() as $item) {
                ...
                $currentOrderItem = $order->getItemByQuoteItemId($quoteItemId);
                if(!$currentOrderItem) {
                    continue;
                }
                $currentOrderItem->setStoreId($item->getStoreId())
                ...
                $currentOrderItem->setProductOptions($options);
            }
        ...
        try {
            $transaction->save();
            if(!Mage::getSingleton('paypal/session')->getData('forceCreateOrder')){
                $this->_inactivateQuote(); // set inactive
            }
