    ...
    public function savePaymentAction()
    {
        ...
        try {
            ...
            if (Mage::getSingleton('customer/session')->isLoggedIn()) {
                ...
                if($data['omise_card_id'] && $data['new_omise_card'] != 'new_omise_card') {
                    $currentCustomerApi = OmiseCustomer::retrieve($customer->getCustomerApiId());
                    $data['omise_customer_id'] = $currentCustomerApi['id'];
                } elseif($data['is_saved_card'] && $data['omise_token']){
                    if($customer->getCustomerApiId()){
                        ...
                        $cardList = $customerApi['cards']['data'];
                        $newCard = end($cardList);
                        $data['omise_card_id'] = $newCard['id'];
                    }else{
                    ...
                    $data['omise_customer_id'] = $customerApi['id'];
                    // $data['omise_customer_id'] = $createCustomerApi['id'];
                }elseif(!$data['is_saved_card'] && $data['omise_token']){
                    if($customer->getCustomerApiId()){
                        $currentCustomerApi = OmiseCustomer::retrieve($customer->getCustomerApiId());
                        $data['omise_customer_id'] = $currentCustomerApi['id'];
                        $data['not_saved_card_token'] = $data['omise_token'];
                    }
                }
            }
    ...
    public function saveOrderAction()
    {
        ...
        try {
            ...
            if ($data) {
                ...
                if($data['method'] = 'p2c2p'){
                    // $p2c2p_request_url   = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB) . 'p2c2p/payment/requestonsite';
                    $p2c2pRequestUrl   = Mage::getUrl('p2c2p/payment/requestonsite');
                    $result['p2c2pRequestUrl'] = $p2c2pRequestUrl;
                }
            }
