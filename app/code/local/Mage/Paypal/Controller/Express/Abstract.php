...
abstract class Mage_Paypal_Controller_Express_Abstract extends Mage_Core_Controller_Front_Action
{
    ...
    public function startAction()
    {
        try {
            ...
            if ($token && $url = $this->_checkout->getRedirectUrl()) {
                ...
                // $this->_initCheckout();
                $this->_getSession()->setData('forceCreateOrder',1);
                $this->_checkout->place($token);
                $this->_getSession()->unsetData('forceCreateOrder');
    ...
    public function placeOrderAction()
    {
        try {
            ...
            Mage::helper('checkout')->sendPaymentFailedEmail($this->_getQuote(), $e->getMessage()); // vao day de ve review
            $this->_getSession()->addError($e->getMessage());
            $this->_redirect('*/*/review');
        } catch (Exception $e) {
    ...
    protected function _initToken($setToken = null)
    {
        ...
        if ($setToken = $this->getRequest()->getParam('token')) { //khi place order o paypal thi vao day
