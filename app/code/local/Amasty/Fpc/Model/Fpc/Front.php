...
AJAX;
    ...
    protected function getPageBlocks($page, $names)
    {
        ...
        if (preg_match_all('|<amfpc(_ajax)? name="(?P<name>[^"]+)"\s*/>|', $page, $matches))
        {
            ...
            foreach ($matches['name'] as $blockNames){
                if(substr($blockNames,0,21) == 'carreservation.label.'){
                    $blocks[] = $blockNames;
                }
            }
        }
    ...
    public function checkCustomerQueue()
    {
        ...
        }else
        {
            ...
            Mage::app()->getResponse()->setRedirect($request->getScheme() . '://' . $request->getHttpHost().'/landing.php?ref_link='.urlencode(Mage::helper('core/url')->getCurrentUrl()))->sendResponse();
            $helper = new Amasty_Base_Helper_Utils();
            $helper->_exit();
            return;
        }
