        ...
        if ($dynamicBlocks = Mage::registry('amfpc_blocks')) {
            ...
            foreach ($dynamicBlocks['blocks'] as $name) {
                ...
                if (!isset($this->_blockHtmlCache[$realName])) {
                    ...
                    }elseif(substr($realName,0,21) == 'carreservation.label.' ){
                        $productId = substr($name,21,strlen($realName));
                        $cartreservationBlock = $layout->createBlock('cartreservation/product','carreservation.label.' . $productId);
                        $cartreservationBlock->setProductId($productId)->setTemplate('cartreservation/reserved.phtml');

                        $this->_blockHtmlCache[$realName] = $cartreservationBlock->toHtml();
                    }
                }
                ...
                if ($cmsAjaxBlocks) {
                    ...
                    foreach ($blocks as $name) {
                        ...
                        if ($block) {
                            ...
                            $result[$name] = $content;
                        }elseif(substr($name,0,21) == 'carreservation.label.' ){
                            $productId = substr($name,21,strlen($name));
                            $cartreservationBlock = $layout->createBlock('cartreservation/product','carreservation.label.' . $productId);
                            $cartreservationBlock->setProductId($productId)->setTemplate('cartreservation/reserved.phtml');

                            $content = Mage::getSingleton('core/url')->sessionUrlVar($cartreservationBlock->toHtml());
