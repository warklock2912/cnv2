 ...
 */
class Amasty_SeoShortUrl_Helper_Url extends Mage_Core_Helper_Abstract
{
    ...
    protected function _getFirstAttributeValueUrl()
    {
        ...
        /** @var Amasty_SeoShortUrl_Helper_Data $dataHelper */
        $dataHelper = Mage::helper('amseoshorturl');
    ...
    /**
     * @deprecated Now it is a facade to Amasty_SeoShortUrl_Model_Url
     */
    public function getFullUrl($query=array(), $clear=false, $cat = null)
    {
        /** @var Amasty_SeoShortUrl_Model_Url_Builder $builder */
        $builder = Mage::getModel('amseoshorturl/url_builder');
        $builder->reset();
    ...
    public function saveParams($request)
    {
        ...
        $currentParams = Mage::registry('amseoshorturl_current_params');
        if (!$currentParams){
            return true;
        }

        // brand-amd-canon/price-100,200 or  amd-canon/100,200
        $hideAttributeNames = Mage::getStoreConfig('amseoshorturl/seo/hide_attributes');

        foreach ($currentParams as $params){

            $attrCode = '';

            $params   = explode(Mage::getStoreConfig('amseoshorturl/seo/option_char'), $params);
            $firstOpt = $params[0];
            ...
            if ($attrCode && isset($options[$attrCode])){
                ...
                if ($this->isDecimal($attrCode)){
                    ...
                    $v = $params[0];
                    if (count($params) > 1){
                        $v = $params[0] . Mage::getStoreConfig('amseoshorturl/seo/option_char') . $params[1];
                    }
    ...
    public function isDecimal($attrCode)
    {
        ...
        /** @var Amasty_SeoShortUrl_Helper_Attributes $attributeHelper */
        $attributeHelper = Mage::helper('amseoshorturl/attributes');
    ...
    {
        return Mage::helper('amseoshorturl/attributes')->getAllFilterableOptionsAsHash();
    }
    ...
    private function _convertIdToKeys($options, $ids)
    {
        ...
        return join(Mage::getStoreConfig('amseoshorturl/seo/option_char'), $keys);
    }

    public function _formatAttributePartMultilevel($attrCode, $ids)
    {
        if ($this->isDecimal($attrCode)){
            return $attrCode . Mage::getStoreConfig('amseoshorturl/seo/option_char') . $ids; // always show price and other decimal attributes
        }
        ...
        $hideAttributeNames = Mage::getStoreConfig('amseoshorturl/seo/hide_attributes');
        $part =  $hideAttributeNames ? $part : ($attrCode . Mage::getStoreConfig('amseoshorturl/seo/option_char') . $part);
    ...
    {
        if ($this->isDecimal($attrCode)){
            return $attrCode . Mage::getStoreConfig('amseoshorturl/seo/option_char') . $ids; // always show other decimal attributes
        }
    ...
    {
        return str_replace(array(Mage::getStoreConfig('amseoshorturl/seo/option_char'), Mage::getStoreConfig('amseoshorturl/seo/special_char')), '_', $attrCode);
    }
