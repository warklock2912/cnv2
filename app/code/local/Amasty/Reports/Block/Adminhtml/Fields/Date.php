...
class Amasty_Reports_Block_Adminhtml_Fields_Date extends Varien_Data_Form_Element_Abstract
{
    /**
     * @var Zend_Date
     */
    ...
    public function getElementHtml()
    {
        ...
        $html .= sprintf(
            '<div style="float: left" class="value"><img src="%s" alt="" class="v-middle report-img" id="%s_trig" title="%s" style="%s" /><input name="%s" id="%s" value="%s" %s style="width:110px !important;" />'
            .' </div>',
        ...
        $this->setValue($this->getData('dateTo'));
        $html .= sprintf(
            '<div style="float: left" class="value"><img src="%s" alt="" class="v-middle report-img" id="%s_trig" title="%s" style="%s" /><input name="%s" id="%s" value="%s" %s style="width:110px !important; margin-left: 9px" />'
            .' </div>',
            $this->getImage(), $this->getData('idTo'), 'Select Date', ($this->getDisabled() ? 'display:none;' : ''),
            $this->getData('nameTo'), $this->getData('idTo'), $this->_escape($this->getValue()), $this->serialize($this->getHtmlAttributes())
        );
        $outputFormat = $this->getFormat();
        if (empty($outputFormat)) {
            throw new Exception('Output format is not specified. Please, specify "format" key in constructor, or set it using setFormat().');
        ...
        $html .= sprintf('
            ...
            $this->getData('idTo'), $displayFormat,
            $this->getTime() ? 'true' : 'false', $this->getData('idTo')
        );
