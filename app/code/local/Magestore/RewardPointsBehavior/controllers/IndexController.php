...
class Magestore_RewardPointsBehavior_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function tweetAction() {        
        ...
        if(!Mage::helper('rewardpointsbehavior')->canEarnPoint('twitter', $customer, $link)){
            $this->getResponse()->setBody(Mage::helper('rewardpointsbehavior')->__('You can not earn points for this action.'));
            return;
        }
        ...
        try {
            ...
            $this->getResponse()->setBody(Mage::helper('rewardpointsbehavior')->__('You have just earned %s for tweeting this via Twitter.', Mage::helper('rewardpoints/point')->format($earnPoint)));
            return;
        } catch (Exception $exc) {
    ...
    public function faceshareAction() {        
        ...
        try {
            ...
            $this->getResponse()->setBody(Mage::helper('rewardpointsbehavior')->__('You have just earned %s for sharing this via Facebook.', Mage::helper('rewardpoints/point')->format($earnPoint)));
            return;
        } catch (Exception $exc) {
            $this->getResponse()->setBody($exc->getMessage());
            return;
    ...
    public function facebookAction() {
        ...
        if(!$remove){
            ...
            try {
                ...
                $this->getResponse()->setBody(Mage::helper('rewardpointsbehavior')->__('You have just earned %s for liking this via Facebook.', Mage::helper('rewardpoints/point')->format($earnPoint)));
                return;
            } catch (Exception $exc) {
                $this->getResponse()->setBody($exc->getMessage());
                return;
    ...
    public function googleplusAction() {
        ...
        if($remove == 'on'){
            ...
            try {
                ...
                $this->getResponse()->setBody(Mage::helper('rewardpointsbehavior')->__('You have just earned %s for +1 via Google+.', Mage::helper('rewardpoints/point')->format($earnPoint)));
                return;
            } catch (Exception $exc) {
                $this->getResponse()->setBody($exc->getMessage());
                return;
	...
	$this->_redirectUrl($data['image']);
        ...
        if (preg_match('/^Pinterest+.*$/', $userAgent)) {
            ...
            try {
                ...
                $this->getResponse()->setBody(Mage::helper('rewardpointsbehavior')->__('You have just earned %s for pin this via Pinterest.', Mage::helper('rewardpoints/point')->format($earnPoint)));
                return;
            } catch (Exception $exc) {
                $this->getResponse()->setBody($exc->getMessage());
                return;
