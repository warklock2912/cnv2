...
class Magestore_RewardPointsRule_Model_Frontend_Observer {
    ...
    public function showEarningOnCategory($observer) {
        ...
        if ($block instanceof Mage_Catalog_Block_Product_Price) {
            ...
            if($_product->getRewardpointsSpend() && $requestPath == 'catalog_product_view' && strpos($transport->getHtml(),Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->getSymbol())!==false){
                $html = '<div class="price-box">
                            <span id="fix-product-price-'.$_product->getId().'" class="regular-price">
                                <span class="icon-coins"></span>
                                <span style="text-transform: uppercase;font-size: 12px; font-weight: bold; font-family: HelveticaNeue;">' .Mage::helper('rewardpoints')->__('Use: '). '</span>
                                <span class="price" style="color: orange">'.Mage::helper('rewardpoints')->__('%s',Mage::helper('rewardpoints/point')->format($_product->getRewardpointsSpend(),Mage::app()->getStore()->getStoreId())).'</span>
                            </span>
                        </div>';
                $transport->setHtml($html);
                return $this;
            }
            if ($requestPath != 'catalog_category_view' && $requestPath != 'catalogsearch_result_index' && $requestPath != 'amshopby_index_index') {
                return;
            }
            $points = 0;
            if($_product->getRewardpointsEarn() != null){
                $points = $_product->getRewardpointsEarn();
            } else {
...
//            if (!$points || $points <= 0) return $this;
            ...
            if($_product->getRewardpointsSpend()){
                $html = '<div class="price-box">
                            <span id="product-price-'.$_product->getId().'" class="regular-price">
                                <span class="icon-coins"></span>
                                <span style="text-transform: uppercase;font-size: 12px; font-weight: bold; font-family: HelveticaNeue;">' .Mage::helper('rewardpoints')->__('Use: '). '</span>
                                <span class="price" style="color: orange">'.Mage::helper('rewardpoints')->__('%s',Mage::helper('rewardpoints/point')->format(
                        $_product->getRewardpointsSpend(), Mage::app()->getStore()->getStoreId())).'</span>
                            </span>
                        </div>';
                $transport->setHtml($html);
                return $this;
    ...
    public function sales_quote_item_save_before($observer){
        ...
        $qty = (int)($customerPoints/$_product->getRewardpointsSpend());
        ...
        if($qty){
            ...
            $item->setRewardpointsSpent($item->getQty() * $_product->getRewardpointsSpend());
            ...
            $catalogRules[$item->getId()] = array(
                ...
                'rule_id'   => 0,
                'point_used'    => $_product->getRewardpointsSpend(),
