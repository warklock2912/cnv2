                ...
                try {
                    ...
                    if ($product && $product->getId()) {
                        if($product->getTypeId() == 'configurable'){
                            $product->setData('rewardpoints_spend', $point);
                            $product->getResource()->saveAttribute($product, 'rewardpoints_spend');
                            $childProducts = Mage::getModel('catalog/product_type_configurable')
                                ->getUsedProducts(null, $product);
                            foreach ($childProducts as $child) {
                                $productChild = Mage::getModel('catalog/product')->load($child->getId());
                                $productChild->setData('rewardpoints_spend', $point);
                                $product->getResource()->saveAttribute($productChild, 'rewardpoints_spend');
                            }
                        }
                        else {
                        ...
                        $out['value'] = Mage::getModel('catalog/product')->loadByAttribute('sku', $product->getSku())->getRewardpointsSpend();
                    } else {
            ...
            try {
                ...
                foreach($productIds as $id) {
                    ...
                    if($product->getTypeId() == 'configurable'){
                        ...
                        foreach ($childProducts as $child) {
                            $productChild = Mage::getModel('catalog/product')->load($child->getId());
                            $productChild->setData('rewardpoints_earn', $point);
                            $product->getResource()->saveAttribute($productChild, 'rewardpoints_spend');
                        }
                    }
                    else{
