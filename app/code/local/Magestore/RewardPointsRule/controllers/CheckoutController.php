...
class Magestore_RewardPointsRule_CheckoutController extends Mage_Core_Controller_Front_Action
{
    ...
    public function priceOptionAction(){
        ...
        if($points > 0)
            $this->getResponse()->setBody(Mage::helper('rewardpoints/point')->getImageHtml(true).' '.$this->__('You will earn %s for purchasing this product.', Mage::helper('rewardpoints/point')->format($points)));
        else $this->getResponse()->setBody('false');
        return;
    }
