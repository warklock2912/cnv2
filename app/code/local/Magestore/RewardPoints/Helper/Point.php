...
class Magestore_RewardPoints_Helper_Point extends Mage_Core_Helper_Abstract
{
     ...
     * @return string
     */
    public function format($points, $store = null, $isNotDisplayed = null)
    {
        $points = intval($points);
        if (abs($points) <= 1) {
            return $points . ' ' . '<span class="span-text" style="text-transform: uppercase; font-size: 12px">'.$this->getName($store) . '</span> ';
        }
        if($isNotDisplayed) {
            return $points;
        }
        return $points . ' ' . '<span class="span-text" style="text-transform: uppercase; font-size: 12px">'.$this->getPluralName($store) . '</span> ';
    }

    public function formatHtml($points, $store = null)
    {
        $points = intval($points);
        if (abs($points) <= 1) {
            return '<span class="span-points">'.$points . '</span> ' . $this->getName($store);
        }
        return '<span class="span-points">'.$points . '</span> ' . $this->getPluralName($store);
    }
