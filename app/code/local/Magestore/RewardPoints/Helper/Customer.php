...
//        }
    ...
    public function getBalanceFormatedHtml(){
        return Mage::helper('rewardpoints/point')->formatHtml(
          $this->getBalance(), $this->getStoreId()
        );
    }


    public function getBalanceFormatedPoints(){
        $checkoutSession = Mage::getSingleton('checkout/session');
        $pointUsing = $checkoutSession->getPointsUsing() ? $checkoutSession->getPointsUsing() : 0;
        $pointSpendProduct = $this->getProductSpendPoints();
        return $this->getBalance() - $pointSpendProduct - intval($pointUsing);
    }

    public function getProductSpendPoints()
    {
        $pointSpendProduct = 0;
        if ($this->getCustomerId()) {
            $quote = Mage::getModel('sales/quote')->loadByCustomer($this->getCustomerId());
            if ($quote) {
                foreach ($quote->getAllVisibleItems() as $item) {
                    $product = Mage::getModel('catalog/product')->loadByAttribute('sku', $item->getSku());
                    if($product->getRewardpointsSpend()){
                        $pointSpendProduct += $product->getRewardpointsSpend() * $item->getQty();
                    }
                }
            }
        }
        return $pointSpendProduct;
    }
    /**
     * get string of points balance formated
