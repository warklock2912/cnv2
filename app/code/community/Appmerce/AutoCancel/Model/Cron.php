...
class Appmerce_AutoCancel_Model_Cron extends Mage_Core_Helper_Abstract
{
    ...
    public function autoCancel($shedule = null)
    {
        ...
        $from = date('Y-m-d H:i:s', $gmtStamp - (168 * 60 * 60));
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (isset($mappings[$method_status]) && !empty($mappings[$method_status])) {
                if (strpos($mappings[$method_status],$this->_divider)){
                    $mapPeriod = explode($this->_divider, $mappings[$method_status]);
                    $time_limit = date('Y-m-d H:i:s', $gmtStamp - ($mapPeriod[1]*24*60));
                } else{
