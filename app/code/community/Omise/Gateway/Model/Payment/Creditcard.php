...
class Omise_Gateway_Model_Payment_Creditcard extends Omise_Gateway_Model_Payment
{
    ...
    public function initialize($payment_action, $state_object)
    {
        ...
        switch ($payment_action) {
            ...
            case Mage_Payment_Model_Method_Abstract::ACTION_AUTHORIZE_CAPTURE:
                ////when create by raffle only AUTHORIZE
                if(Mage::app()->getRequest()->getControllerName()=='OrderRuffle' && Mage::app()->getRequest()->getActionName()=='createOrder')
                {
                    $charge = $this->processPaymentRuffle($payment, $order->getBaseTotalDue());
    ...
    public function processPayment(Varien_Object $payment, $amount)
    {
        ...
        if ($payment->getAdditionalInformation('not_saved_card_token') || !$payment->getAdditionalInformation('omise_customer_id')) {
            ...
            if ($payment->getAdditionalInformation('omise_card_id')) {
                return $this->_process(
                    $payment,
                    array(
                        'amount' => $this->getAmountInSubunits($amount, $order->getBaseCurrencyCode()),
                        ...
                        'description' => 'Charge a card from Magento with the order id: '.$order->getIncrementId(),
                        'capture' => $this->isAutoCapture() ? true : false,
                        'customer' => $payment->getAdditionalInformation('omise_customer_id'),
                        'card' => $payment->getAdditionalInformation('omise_card_id'),
                        'return_uri' => $this->getThreeDSecureCallbackUri(),
                        'metadata' => array(
                            'order_id' => $order->getIncrementId(),
                        ),
    ...
    public function capture_manual(Varien_Object $payment, $charge_id)
    {
        ...
        ///if already capture we no need to capture anymore
    ...
    public function assignData($data)
    {
        ...
        if (is_array($data)) {
            if (!isset($data['omise_token']) && !isset($data['omise_customer_id']) && !isset($data['omise_card_id'])) {
            ...
            if (isset($data['omise_card_id'])) {
                $this->getInfoInstance()->setAdditionalInformation('omise_card_id', $data['omise_card_id']);
            }
            if (isset($data['omise_customer_id'])) {
                $this->getInfoInstance()->setAdditionalInformation('omise_customer_id', $data['omise_customer_id']);
            }
            if (isset($data['not_saved_card_token'])) {
                $this->getInfoInstance()->setAdditionalInformation('not_saved_card_token', $data['not_saved_card_token']);
            }
            $this->getInfoInstance()->setAdditionalInformation('omise_token', $data['omise_token']);
            //
            // $this->getInfoInstance()->setAdditionalInformation('omise_card_id', $data['omise_card_id']);
        } elseif ($data instanceof Varien_Object) {
            if (!$data->getData('omise_token') && !$data->getData('omise_customer_id') && !$data->getData('omise_card_id')) {
                Mage::throwException(Mage::helper('payment')->__('Cannot retrieve your credit card information. Please make sure that you entered your card information correctly, or contact our support team if you have any questions.'));
            }
            Mage::log('Data that assign is Object');
            $this->getInfoInstance()->setAdditionalInformation('omise_token', $data->getData('omise_token'));
            if ($data->getData('omise_customer_id')) {
                $this->getInfoInstance()->setAdditionalInformation('omise_customer_id', $data->getData('omise_customer_id'));
            }
            if ($data->getData('omise_card_id')) {
                $this->getInfoInstance()->setAdditionalInformation('omise_card_id', $data->getData('omise_card_id'));
            }
            if ($data->getData('not_saved_card_token')) {
                $this->getInfoInstance()->setAdditionalInformation('not_saved_card_token', $data->getData('not_saved_card_token'));
            }
        }
    ...
    public function processPaymentRuffle(Varien_Object $payment, $amount)
    {
        $order = $payment->getOrder();
        $omiseToken = $payment->getAdditionalInformation('omise_token');
        $omise_customer_id = $payment->getAdditionalInformation('omise_customer_id');
        $omise_card_id = $payment->getAdditionalInformation('omise_card_id');
        if($omiseToken){
            return $this->_process(
                $payment,
                array(
