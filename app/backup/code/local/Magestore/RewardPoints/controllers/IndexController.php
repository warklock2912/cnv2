...
class Magestore_RewardPoints_IndexController extends Mage_Core_Controller_Front_Action {
  ...
  public function importPointForCustomerAction(){
    $file = Mage::getBaseDir().DS.'customer_point.csv';
    $csv = new Varien_File_Csv();
    $data = $csv->getData($file);
    foreach($data as $key => $value){
      if($key != 0){
        if($value[0]){
          $customer = Mage::getModel('customer/customer')->setWebsiteId(1)->loadByEmail($value[0]);
          if($customer->getData()){
            $this->updateCustomerPOint($customer, $value[1]);
          }
        }
      }
    }
  ...
  public function updateCustomerPOint($customer, $point){
    $params = array();
