 ...
 */ 
class Amasty_Payrestriction_Adminhtml_Ampayrestriction_RuleController extends Mage_Adminhtml_Controller_Action
{
    protected $_title     = 'Payment Restriction';
    ...
    {
	    $this->loadLayout(); 
        $this->_setActiveMenu('sales/ampayrestriction/' . $this->_modelName . 's');
        $this->_addContent($this->getLayout()->createBlock('ampayrestriction/adminhtml_' . $this->_modelName)); 	    
 	    $this->renderLayout();
    }
    ...
    public function editAction() 
    {
		...
		$model  = Mage::getModel('ampayrestriction/' . $this->_modelName)->load($id);

		if ($id && !$model->getId()) {
    		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('ampayrestriction')->__('Record does not exist'));
			$this->_redirect('*/*/');
			return;
		}
		...
		Mage::register('ampayrestriction_' . $this->_modelName, $model);
        ...
        $this->_addContent($this->getLayout()->createBlock('ampayrestriction/adminhtml_' . $this->_modelName . '_edit'));
        $this->_addLeft($this->getLayout()->createBlock('ampayrestriction/adminhtml_' . $this->_modelName . '_edit_tabs'));
	...
	public function saveAction() 
	{
	    ...
	    $model  = Mage::getModel('ampayrestriction/' . $this->_modelName);
		...
		if ($data) {
			...
			try {
				...
				$msg = Mage::helper('ampayrestriction')->__($this->_title . ' has been successfully saved');
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('ampayrestriction')->__('Unable to find a record to save'));
        $this->_redirect('*/*');
	} 
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($ids)) {
             Mage::getSingleton('adminhtml/session')->addError(Mage::helper('ampayrestriction')->__('Please select records'));
             $this->_redirect('*/*/');
             return;
        }
    ...
    protected function _modifyStatus($status)
    {
        ...
        if ($ids && is_array($ids)){
            try {
                Mage::getModel('ampayrestriction/' . $this->_modelName)->massChangeStatus($ids, $status);
    ...
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('sales/ampayrestriction');
    }
}