...
class Amasty_SeoShortUrl_Helper_Data extends Amasty_SeoShortUrl_Helper_Cached
{
    protected $_useSolr;
    
    const XML_PATH_SEO_PRICE_NOFOLLOW       = 'amseoshorturl/seo/price_nofollow';
    const XML_PATH_SEO_PRICE_NOINDEX        = 'amseoshorturl/seo/price_noindex';
    const XML_PATH_SEO_PRICE_RELNOFOLLOW    = 'amseoshorturl/seo/price_rel_nofollow';
    ...
    protected function _getFilters()
    {
        ...
        $filterCollection = Mage::getResourceModel('amseoshorturl/filter_collection')
    ...
    public function getAttributesSettings()
    {
        ...
        $attrCollection = Mage::getResourceModel('amseoshorturl/filter_collection')->load();
    ...
    public function getIconsData()
    {
        ...
        $optionCollection = Mage::getResourceModel('amseoshorturl/value_collection')
            ->addPositions()
        ...
        $hlp = Mage::helper('amseoshorturl/url');
        foreach ($optionCollection as $opt){
            ...
            $result[$opt->getOptionId()] = array(
                ...
                'title' => $opt->getTitle(),
                'descr' => $opt->getDescr(),
                'img'   => Mage::getBaseUrl('media') . 'amseoshorturl/' . $img,
    ...
    public function showLinks($product, $mode='view', $names=array(), $exclude=false)
    {
        ...
        $block = Mage::getModel('core/layout')->createBlock('core/template')
            ->setArea('frontend')
            ->setTemplate('amasty/amseoshorturl/links.phtml');
    ...
    protected function getMultiselectAttributeCodes()
    {
        ...
        $attributesHelper = Mage::helper('amseoshorturl/attributes');
    ...
    public function useSolr()
    {
        ...
        if ($this->isModuleEnabled('Enterprise_Search')) {
            ...
            if ($routeName == 'catalog' || $routeName == 'amseoshorturl') {
    ...
    public function getCurrentCategory()
    {
        ...
        $forwardedCategoryId = Mage::registry('amseoshorturl_forwarded_category_id');
    ...
    public function isDeniedModule()
    {
        return
            !Mage::getStoreConfig('amseoshorturl/general/active')
            || Mage::helper(
                'amseoshorturl'
            )->isModuleEnabled('Amasty_Shopby');
    }
}
