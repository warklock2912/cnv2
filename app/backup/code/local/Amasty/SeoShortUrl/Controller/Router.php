 ...
 */
class Amasty_SeoShortUrl_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    const MIDDLE    = 0;
    const BEGINNING = 1;
    ...
    protected function matchForwardShort()
    {
        ...
        /** @var Amasty_SeoShortUrl_Model_Url_Parser $shortParser */
        $shortParser = Mage::getModel('amseoshorturl/url_parser');
    ...
    protected function matchForwardMultilevel()
    {
        ...
        if ($this->urlMode == Amasty_SeoShortUrl_Model_Source_Url_Mode::MODE_SHORT && Mage::getStoreConfig('amseoshorturl/seo/redirects_enabled')) {
    ...
    protected function matchMultilevel($pageId)
    {
        ...
        //  electronics/seourl/canon/
        //  electronics/seourl/red/
        //  electronics/seourl/

        //  seourl/
        //  seourl/red/
        //  seourl/canon/ - false
        //  seourl/manufacturer-canon/ - false
        //  manufacturer-canon/ - true

        // starts from seourl
        $isAllProductsPage = substr($pageId, 0, strlen($reservedKey)) == $reservedKey;

        // has seourl in the middle
        ...
        if ($params || $params === '0'){
            Mage::register('amseoshorturl_current_params', $params);
    ...
    {
        $code = Mage::getStoreConfig('amseoshorturl/brands/attr');
        $code = trim(str_replace('_', Mage::getStoreConfig('amseoshorturl/seo/special_char'), $code));
        ...
        foreach ($options[$code] as $key => $id) {
            if (!Mage::getStoreConfig('amseoshorturl/seo/hide_attributes')){
                $key = $code . Mage::getStoreConfig('amseoshorturl/seo/option_char') . $key;
    ...
    protected function forwardCategory($cat)
    {
        ...
        $isVersionEE13 = Mage::helper('amseoshorturl')->isModuleEnabled('Enterprise_UrlRewrite');
        ...
        Mage::register('amseoshorturl_forwarded_category_id', $categoryId);
    }

    protected function forwardSeoShortUrl()
    {
        $reservedKey = Mage::getStoreConfig('amseoshorturl/seo/key');
        $realModule = 'Amasty_SeoShortUrl';

        $this->request->setPathInfo($reservedKey);
        $this->request->setModuleName('amseoshorturl');
        $this->request->setRouteName('amseoshorturl');
