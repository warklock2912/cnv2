...
class Amasty_Shopby_Helper_Data extends Amasty_Shopby_Helper_Cached
{
    ...
    public function getBrandName($item)
    {
      $product = Mage::getModel('catalog/product')->load($item->getProductId());
      return $product->getAttributeText('brands');
    }
    public function getPriceProduct($item)
    {
      $html = '';
      $product = Mage::getModel('catalog/product')->load($item->getProductId());
      if($product->getFinalPrice() < $product->getPrice()){
        $html .= '<div class="price-box">'; 
        $html .= '<p class="special-price">'.Mage::helper('core')->currency($product->getFinalPrice());
        $html .= '</p><p class="old-price">'.Mage::helper('core')->currency($product->getPrice()); 
        $html .= '</p></div>'; 
      }else{
        $html .= '<div class="price-box">'; 
        $html .= '<p class="regular-price">'.Mage::helper('core')->currency($product->getPrice()); 
        $html .= '</p></div>'; 
      }
      return $html;
    }
    public function getImageProduct($item)
    {
        $product = Mage::getModel('catalog/product')->load($item->getProductId());
        return Mage::helper('catalog/image')->init($product, 'small_image')->keepFrame(false)->resize(180,180);
    }
    public function getUrlProduct($item)
    {
        $product = Mage::getModel('catalog/product')->load($item->getProductId());
        return $product->getProductUrl();
    }

  public function getPromitionHtml($_product, $promotion){
    $html = '';

    $attributeId = Mage::getResourceModel('eav/entity_attribute')
      ->getIdByCode('catalog_product', 'promotion');

    $filterId = Mage::getModel('amshopby/filter')->getResourceCollection()
      ->addFieldToFilter('attribute_id', $attributeId)->getFirstItem()->getFilterId();
    if(is_array($promotion)){
      $arrayOptionId = array();
        foreach($promotion as $key => $value){
          $arrayOptionId[] = $_product->getResource()->getAttribute('promotion')->getSource()->getOptionId($value);
        }
      $collectionValues = Mage::getResourceModel('amshopby/value_collection')
        ->addFieldToFilter('filter_id', $filterId)
        ->addFieldToFilter('option_id', array('in' => $arrayOptionId));
      if($collectionValues->getData()){
        foreach($collectionValues as $_value){
          $pathImage = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'amshopby/'.$_value->getData('img_small');
          $html .= '<table onclick="window.location='.$_product->getProductUrl().'" class="amlabel-table top-left" style="height:35px; width:35px; ">
                     <tbody>
                       <tr>
                        <td style="background: url('.$pathImage.') no-repeat 0 0">
      ...
      $OptionId = $_product->getResource()->getAttribute('promotion')->getSource()->getOptionId($promotion);
      $collectionValues = Mage::getResourceModel('amshopby/value_collection')
        ->addFieldToFilter('filter_id', $filterId)
