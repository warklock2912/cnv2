...
*/
class Magebuzz_Customaddress_Adminhtml_Customaddress_RegionController extends Mage_Adminhtml_Controller_Action {
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('customaddress/region')
	...
	public function editAction() {
		...
		$model = Mage::getModel('customaddress/region')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('customaddress/region');
			...
			$this->_addContent($this->getLayout()->createBlock('customaddress/adminhtml_region_edit'))
				->_addLeft($this->getLayout()->createBlock('customaddress/adminhtml_region_edit_tabs'));
	...
	public function saveAction() {
		...
		if ($this->getRequest()->getPost()) {
			...
			$state = Mage::getModel('customaddress/region')->getCollection()
			...
			try {
				$state = Mage::getModel('customaddress/region');
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$model = Mage::getModel('customaddress/region');
		...
		if(!is_array($regionIds)) {
			...
			try {
				foreach ($regionIds as $regionId) {
					$region = Mage::getModel('customaddress/region')->load($regionId);
					$region->delete();
				}
	...
	protected function _isAllowed()	{
		return Mage::getSingleton('admin/session')->isAllowed('customaddress/region');
	}
}