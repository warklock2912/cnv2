...
class Omise_Gateway_Block_Form_Cc extends Mage_Payment_Block_Form
{
    ...
    public function isHasToken(){
        if(Mage::getStoreConfig('payment/omise_gateway/usetoken')){
            ...
            $lasttoken = $token->load($customer->getCustomerId(),'customer_id');
            if($lasttoken->getId()){
                return true;
            } else {
                return false;
    ...
    public function isUsePopup(){
        return Mage::getStoreConfig('payment/omise_gateway/usepopup');
    }
    /**
