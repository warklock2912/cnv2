...
Payment.prototype = {
    ...
    nextStep: function(transport){
        ...
        if(paymentMethod === 'kpayment_credit'){
            var popup = jQuery('#kpayment_popup');
            popup.show();
        }
        checkout.setStepResponse(response);
...
Review.prototype = {
    ...
    nextStep: function(transport){
        if (transport && transport.responseText) {
            ...
            if (response.redirect) {
                if(response.redirect.includes('p2c2p/payment/redirect')=== true)
                {
                    var requestUrl = response.p2c2pRequestUrl;
                    var  f= document.createElement("form");
                    var getUrl = window.location;
                    var baseUrl = getUrl.protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
                    // var requestUrl= baseUrl+"/p2c2p/payment/requestonsite";
                    f.setAttribute('method',"post");
                    f.setAttribute('action',requestUrl);
                    f.setAttribute('id',"2c2p-payment-form");
                    jQuery("#payment_form_p2c2p").find('input').each(function(){
                        f.append(this);
                    });
                    jQuery("#payment_form_p2c2p").find('select').each(function(){
                        f.append(this);
                    });
