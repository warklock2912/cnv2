...
Product.Config.prototype.fillSelect = function(element){
    ...
    if(options) {
        ...
        if(parseInt(savedValue) && this.settings.length <= 2 && element.select('[value="' + savedValue + '"]').length){
...
Product.Config.prototype.getOptionLabel = function(option, price){
    if ('undefined' == typeof(confData)
        ...
        if ('undefined' == typeof(confData['optionProducts'])
            || 'undefined' == typeof(confData['optionProducts']['dropdownPrice'])
        ...
        if( confData['optionProducts'][keyCheck] && confData['optionProducts'][keyCheck]['price'] ) {
            ...
            if ( type == "2" ) {// show actual price
                ...
                price = simplePrice - selectedPrice;
...
Product.Config.prototype.updateData = function(key)
{
    ...
    { 
        ...
        this.updateSimpleData('name', key);
        this.updateSimpleData('short_description', key);
        this.updateSimpleData('description', key);
        this.updateSimpleData('attributes', key);
        ...
        if (true == confData.getDefault('set'))
        {
            this.getDefaultSimpleData('name');
            this.getDefaultSimpleData('short_description');
            this.getDefaultSimpleData('description');
            this.getDefaultSimpleData('attributes');
...
Product.Config.prototype.updateSimpleData= function(type, key){
    if (confData.getData(key, type))
    {
        var selector = confData.getData('selector', type);
...
Product.Config.prototype.getDefaultSimpleData= function(type) {
    if (confData.getDefault(type))
    {
        var selector = confData.getData('selector', type);
        $$(selector).each(function(container){
